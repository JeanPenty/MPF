...
void DesignPanel::SetElementWidth(DesignElement* elem, Float wid)
{
    SetterNode* pSetter = ElementRootItem::SetElementValue(elem, FrameworkElement::WidthProperty, new OFloat(wid));
    if (NULL != pSetter)
    {
...
void DesignPanel::SetElementHeight(DesignElement* elem, Float hei)
{
    SetterNode* pSetter = ElementRootItem::SetElementValue(elem, FrameworkElement::HeightProperty, new OFloat(hei));
    if (NULL != pSetter)
    {
        UpdateElementSetter(elem, pSetter);
    }
...
void DesignPanel::SetElementMargin(DesignElement* elem, fRect rect)
{
    SetterNode* pSetter = ElementRootItem::SetElementValue(elem, FrameworkElement::MarginProperty, new OfRect(rect));
    if (NULL != pSetter)
    {
        UpdateElementSetter(elem, pSetter);
    }
...
fSize DesignPanel::OnMeasure(const suic::fSize& constraitSize)
{
    if (_root && _root->GetUIElement())
    {
        suic::FrameworkElement* pElem = _root->GetUIElement();
        fSize size = pElem->GetDesiredSize();
        if (_needMeasure)
        {
            _needMeasure = false;
            ...
            if (pWnd != NULL)
            {
                suic::fSize szMeasure = constraitSize;
                if (pWnd->GetSizeToContent() == suic::SizeToContent::stcWidth)
                {
                    szMeasure.cx = suic::FloatUtil::PosInfinity;
                }
                else if (pWnd->GetSizeToContent() == suic::SizeToContent::stcHeight)
                {
                    szMeasure.cy = suic::FloatUtil::PosInfinity;
                }
...
void DesignPanel::OnArrange(const suic::fSize& availableSize)
{
    if (_root && _root->GetUIElement())
    {
        suic::fRect rcLay;
...
Rect DesignPanel::GetRelativeRect(Element* Other)
{
    ...
    return Rect(ptPanel, Other->GetRenderSize().ToSize());
}
...
void DesignPanel::RenderDesignMeta(suic::Drawing* drawing)
{
    ...
    suic::fRect rect(fPoint(), GetRenderSize());
...
void DesignPanel::MouseMoveInAddMode(suic::MouseButtonEventArg* e)
{
    ...
    else 
    {
        ...
        if (fdelem != _addParent && fdelem && !fdelem->IsLocked() && _isAddSucc)
        {
            int x = _trackDragPt.x - _startDragPt.x;
            int y = _trackDragPt.y - _startDragPt.y;
            fRect rcMgr(_startDragMgr.TofRect());
...
void DesignPanel::MLDownInSelectMode(suic::MouseButtonEventArg* e)
{
    ...
    if (_dragMeta && _root->GetFocusElement())
    {
        ...
        _startDragMgr = ele->GetMargin().ToRect();
...
void DesignPanel::MLDownInAddMode(suic::MouseButtonEventArg* e)
{
    ...
    if (CheckCanAddElement(delem) && !delem->IsLocked())
    {
        ...
        if (delem->CreateElement(child, strPath, _elemType))
        {
            ...
            {
                SetElementMargin(child.get(), rcMgr.TofRect());
            }
...
void DesignPanel::OnKeyDown(suic::KeyboardEventArg* e)
{
    ...
    suic::fRect rcMgr = felem->GetMargin();
...
void DesignPanel::DragMoveFinish(DesignElement* dfocus, DesignElement* dparent)
{
    ...
    {
        SetElementMargin(dfocus, rcMgr.TofRect());
...
void DesignPanel::HandleCopyElement(DesignElement* dfocus, suic::IAddChild* mvPanel)
{
    ...
    if (NULL != pCloneElem)
    {
        ...
        SetElementMargin(pCloneElem, rcDraw.TofRect());
        ...
        if (suic::Keyboard::IsControlDown() && suic::Keyboard::IsAltDown())
        {
                ...
                dparent != _dragMouseOver && !IsAncestor(_dragMouseOver, dfocus))
            {
                ...
                if (NULL != newGrid || NULL != newCanvas)
                {
                    ...
                    SetElementMargin(dfocus, rcDraw.TofRect());
                }
                else
                {
...
void DesignPanel::InternalHandleDrag(suic::Rect& rect, int* pHori, int* pVert)
{
    ...
    if (!IsMain())
    {
        ...
        suic::fRect rcMgr = ele->GetMargin();
