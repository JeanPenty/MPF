...
fSize SetterEditor::OnMeasure(const suic::fSize& constraitSize)
{
    fSize size = Control::OnMeasure(constraitSize);
    if (NULL != _operPanel)
    {
...
void SetterEditor::OnArrange(const fSize& arrangeSize)
{
    ...
    {
        Float hei = arrangeSize.Height();
        Float x = fe->GetVisualOffset().x + fe->GetRenderSize().Width() + 3;
        fRect rect(fPoint(x, 0), fSize(_operPanel->GetDesiredSize().Width(), hei));
