...
Float ToolBar::GetMinLength() const
{
    return _minLength;
}

Float ToolBar::GetMaxLength() const
{
    return _maxLength;
}
...
fSize ToolBar::OnMeasure(const fSize& constraint)
{
    fSize size = HeaderedItemsControl::OnMeasure(constraint);
    ToolBarPanel* toolBarPanel = GetToolBarPanel();

    if (toolBarPanel != NULL)
    {
        Float num = 0;
        fRect margin = toolBarPanel->GetMargin();

        if (toolBarPanel->GetOrientation() == Orientation::Horizontal)
        {
            num = Math::Max(0, (((size.Width() - toolBarPanel->GetDesiredSize().Width()) + margin.left) + margin.right));
        }
        else
        {
            num = Math::Max(0, (((size.Height() - toolBarPanel->GetDesiredSize().Height()) + margin.top) + margin.bottom));
        }
...
void ToolBarTray::OnArrange(const fSize& arrangeSize)
{
    ...
    for (int i = 0; i < _bands.Length(); i++)
    {
        ...
        {
            ToolBar* bar = band[j];
            fSize size(flag ? bar->GetDesiredSize().Width() : thickness, flag ? thickness : bar->GetDesiredSize().Height());
...
fSize ToolBarTray::OnMeasure(const fSize& constraint)
{
    GenerateBands();

    fSize size;
    bool flag = GetOrientation() == Orientation::Horizontal;
    fSize availableSize(FloatUtil::PosInfinity, FloatUtil::PosInfinity);

    for (int i = 0; i < _bands.Length(); i++)
    {
        Float num4 = 0;
        Float num5 = 0;
        Float num3 = flag ? constraint.Width() : constraint.Height();
        ...
        for (num2 = 0; num2 < band.Length(); num2++)
        {
            ...
            fSize desiredSize = bar2->GetDesiredSize();
        ...
        {
            size.cy += num4;
            size.cx = Math::Max(size.Width(), num5);
        }
        else
        {
            size.cx += num4;
            size.cy = Math::Max(size.Height(), num5);
        }
    }
