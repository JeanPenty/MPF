...
void HyperPanel::UpdateTransforms(suic::fSize availableSize)
{
    suic::Float num = 0.0f;
...
suic::fSize HyperPanel::OnMeasure(const suic::fSize& availableSize)
{
    suic::Element* pChild = NULL;
    ...
    return suic::fSize(_totalWidth, _totalHeight);
}

suic::fSize HyperPanel::GetScaledSize(suic::Element* elem)
{
    suic::fSize desiredSize = elem->GetDesiredSize();
...
void HyperPanel::OnArrange(const suic::fSize& finalSize)
{
    ...
    for (int i = 0; i < children->GetCount(); i++)
    {
        ...
        suic::fSize scaledSize = GetScaledSize(elem);
        suic::fSize desiredSize = elem->GetDesiredSize();
