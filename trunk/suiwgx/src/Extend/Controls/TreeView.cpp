...
bool TreeView::ComputeItemOffset(ItemCollection* items, bool bHori, Object* item, Float& iOffset, Float& iItemSize)
{
    bool bFind = false;
    ...
    for (int i = 0; i < items->GetCount(); ++i)
    {
        ...
        Float iHeaderSize = DEFAULTITEMSIZE;
        ...
        {
            fSize estimateSize = EstimatedContainerSize(bHori);
...
void TreeView::RegisterContainerSize(Float size)
{
    ...
    if (arrays != NULL)
    {
        ...
        for (int i = 0; i < arrays->GetCount(); i++)
        {
            ...
            if (temp->IsCloseTo(size) && (temp->containerCount < Numeric::MaxInt32))
            {
                temp->containerCount++;
                flag = true;
            }
...
fSize TreeView::EstimatedContainerSize(bool isHorizontal)
{
    fSize size(DEFAULTITEMSIZE, DEFAULTITEMSIZE);
