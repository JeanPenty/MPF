...
fSize ToolBarPanel::OnMeasure(const fSize& constraint)
{
    fSize panelDesiredSize;

    if (IsItemsHost())
    {
        Float width = 0;
        Float num2 = 0;
        fSize size2 = constraint;
        bool bHori = GetOrientation() == Orientation::Horizontal;

        if (bHori)
        {
            size2.cx = FloatUtil::PosInfinity;
            width = constraint.Width();
        }
        else
        {
            size2.cy = FloatUtil::PosInfinity;
...
void ToolBarPanel::OnArrange(const fSize& arrangeSize)
{
    ElementColl* children = GetChildren();
    bool bHori = GetOrientation() == Orientation::Horizontal;
    fRect finalRect(fPoint(), arrangeSize);
    int width = 0;
    int index = 0;
    int count = children->GetCount();
...
bool ToolBarPanel::MeasureGeneratedItems(bool asNeededPass, fSize constraint, 
                                         bool horizontal, Float maxExtent, 
                                         fSize& panelDesiredSize, Float& overflowExtent)
{
    ElementColl* children = GetChildren();
    ToolBarOverflowPanel* toolBarOverflowPanel = GetToolBarOverflowPanel();
...
void ToolBarOverflowPanel::ArrangeLine(int y, int lineHeight, int start, int end)
{
    ...
    for (int i = start; i < end; i++)
    {
        ...
        elem->Arrange(fRect(x, y, elem->GetDesiredSize().Width(), lineHeight));
        x += elem->GetDesiredSize().Width();
    }
}

void ToolBarOverflowPanel::OnArrange(const fSize& arrangeBounds)
{
    fSize size;
    int start = 0;
    int y = 0;
    ...
    {
        fSize desiredSize = children->GetAt(i)->GetDesiredSize();
...
fSize ToolBarOverflowPanel::OnMeasure(const fSize& constraint)
{
    fSize size;
    _panelSize = fSize();
    ...
    for (int j = 0; j < children->GetCount(); j++)
    {
        ...
        fSize size3 = element3->GetDesiredSize();
