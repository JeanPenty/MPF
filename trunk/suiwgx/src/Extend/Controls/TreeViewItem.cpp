...
fSize TreeViewItem::ComputeRenderSize()
{
    fSize size;
...
Float TreeViewItem::GetHeaderSize(bool isHorizontal)
{
    return HeaderedItemsControl::GetHeaderSize(isHorizontal);
}

fSize TreeViewItem::EstimatedContainerSize(bool isHorizontal)
{
    TreeView* parentTreeView = GetParentTreeView();

    if (parentTreeView != NULL)
    {
        fSize size = parentTreeView->EstimatedContainerSize(isHorizontal);
        return size;
    }
    else
    {
        return fSize(DEFAULTITEMSIZE, DEFAULTITEMSIZE);
    }
}
...
fSize TreeViewItem::OnMeasure(const fSize& constraint)
{
    ...
    Float iHeaderSize = GetHeaderSize(bHori);
...
void TreeViewItem::OnMouseLeftButtonDown(MouseButtonEventArg* e)
{
    ...
    suic::Rect rect(PointToScreen(suic::Point()), GetRenderSize().ToSize());
