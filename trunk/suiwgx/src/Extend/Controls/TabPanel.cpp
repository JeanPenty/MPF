...
void TabPanel::OnArrange(const fSize& arrangeSize)
{
    switch (GetTabStripPlacement())
    {
    case Dock::dockTop:
...
void TabPanel::ArrangeVertical(fSize arrangeSize)
{
...
void TabPanel::ArrangeHorizontal(fSize arrangeSize)
{
    if (GetTabArrangeMode() == TabArrangeMode::tamSingle)
    {
        ArrangeSingle(arrangeSize);
...
void TabPanel::ArrangeSingle(fSize arrangeSize)
{
    suic::fSize desiredSize;
    FrameworkElement* elem = NULL;
    ElementColl* elemsColl = GetInternalChildren();
    Dock tabStripPlacement = GetTabStripPlacement();
    suic::fRect rect;
...
void TabPanel::ArrangeMultiHorizontal(fSize arrangeSize)
{
    Dock tabStripPlacement = GetTabStripPlacement();
    ...
    for (int i = 0; i < elemsColl->GetCount(); ++i)
    {
        ...
        fRect thickness = elem->GetMargin();
        ...
        fSize size(headersSize[index], _rowHeight);
        ...
        elem->Arrange(fRect(vector, size));
        fSize size2 = size;
...
void TabPanel::ArrangeMultiVertical(fSize arrangeSize)
{
    float y = 0.0;
    ElementColl* elemsColl = GetInternalChildren();
    ...
    for (int i = 0; i < elemsColl->GetCount(); ++i)
    {
        ...
        if (elem->GetVisibility() != Visibility::Collapsed)
        {
            ...
            elem->Arrange(fRect(0.0, y, arrangeSize.Width(), desiredSizeWithoutMargin.Height()));
...
fSize TabPanel::GetDesiredSizeWithoutMargin(Element* elem)
{
    fRect rcMgr = CASTTOFE(elem)->GetMargin();
    fSize size;
...
fSize TabPanel::OnMeasure(const fSize& constraint)
{
...
fSize TabPanel::MeasureMulti(fSize constraint)
{
    fSize size;
    Dock tabStripPlacement = GetTabStripPlacement();
    _numRows = 1;
...
fSize TabPanel::MeasureMenu(fSize constraint)
{
    return constraint;
}

fSize TabPanel::MeasureSingle(fSize constraint)
{
    int prevSize = 0;
    suic::fSize size;
