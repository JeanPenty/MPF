...
void TextContainer::OnArrange(const suic::fSize& arrangeSize)
{
}
...
{
    suic::fPoint pt = GetCanvasOffset();
    suic::fRect rcClient(pt, GetRenderSize());
    __ToRenderHost()->SetClntRect(rcClient.ToRect());
}
...
suic::fSize TextContainer::OnMeasure(const suic::fSize& constraint)
{
    ...
    if (__ToRenderHost()->GetMaxLines() <= 1)
    {
        ...
        return suic::fSize(tm.size.cy + 2, tm.size.cy + 2);
    }
    else
    {
        return suic::fSize();
