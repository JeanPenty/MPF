...
{ 
    MinMaxInfo();
    void UpdateMinMax(FrameworkElement* fe, Float w, Float h);
...
public:
    ...
    Float CoercedWidth(Float w);
    Float CoercedHeight(Float h);
...
protected:
    ...
    void ArrangeCore(const fRect& arrangeRect);
    fSize MeasureCore(const fSize& availableSize);
    ...
    virtual void UpdateMeasureSize(const fSize& availableSize, fSize& measureSize);

    void DoPropertyAffects(DpProperty* dp, PropMetadata* meta);
    fSize CalcMaximalSpaceRect(const Matrix& matrix, fSize size);

    void UpdateMinMaxInfo(Float w, Float h);
    ...
    fPoint ComputeAlignmentOffset(fSize clientSize, fSize inkSize);
...
inline Float FrameworkElement::GetWidth()
{
    return _width;
}

inline Float FrameworkElement::GetHeight()
{
    return _height;
}

inline Float FrameworkElement::GetMaxWidth()
{
    return GetValue(MaxWidthProperty)->ToFloat();
}

inline Float FrameworkElement:: GetMaxHeight()
{
    return GetValue(MaxHeightProperty)->ToFloat();
}

inline Float FrameworkElement::GetMinWidth()
{
    return GetValue(MinWidthProperty)->ToFloat();
}

inline Float FrameworkElement::GetMinHeight()
{
    return GetValue(MinHeightProperty)->ToFloat();
}

inline fRect FrameworkElement::GetMargin()
{
    return _margin;
}
