...
struct MeasureData
{
    ...
    void SetAvailableSize(fSize size)
    {
        availableSize = size;
    }

    void SetViewPort(fRect rect)
    {
        viewPort = rect;
    }

    fSize GetAvailableSize() const
    {
...
struct LayoutTransformInfo
{
    Matrix transform;
    fPoint offset;
    fSize transformedSize;
    fSize untransformedSize;
};
...
public:
    ...
    /// </param>
    /// <returns>无</returns> 
    void Measure(const fSize& availableSize);
    ...
    /// <returns>无</returns> 
    void Arrange(const fRect& arrangeRect);
...
public:
    ...
    void MeasureChild(Element* child, fSize constraint);
    void SetMeasureDataOnChild(Element* child, fSize constraint);
...
public:
    ...
    fRect& GetDescendantBounds();

    /// <summary>
    ///     获取界面元素相对父元素的偏移。
    /// </summary>
    /// <returns>相对父元素的偏移</returns>
    fPoint GetVisualOffset() const;
    void SetVisualOffset(const fPoint& pt);

    fRect GetViewClip();
    void SetViewClip(const fRect& clip);
...
public:
    ...
    virtual fSize OnMeasure(const fSize& constraintSize);
    ...
    virtual void OnArrange(const fSize& arrangeSize);
    ...
    /// <returns>界面元素期望的大小</returns>
    virtual fSize MeasureCore(const fSize& availableSize);    
    ...
    /// <returns>无</returns>
    virtual void ArrangeCore(const fRect& arrangeRect);
    virtual fSize ComputeRenderSize();
